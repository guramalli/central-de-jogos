...
export function EscadaPatentes({ jogo, mensal, semLink = false, rodape = null }) {
  ...
  return (
    <section className="v2-cartao v2-escada">
      <div className="v2-cartao-cabeca">
        <h2>Patentes do {jogo === "stop" ? "Stop" : jogo === "acromania" ? "Acromania" : jogo === "mentira" ? "Mentira Sincera" : "Quiz"} <DicaNova chave="patentes" texto="Essa é a sua patente do mês. Arraste pro lado pra ver quanto falta pra próxima — ela some e recomeça do zero todo dia 1º." lado="baixo-esquerda" alvoSeletor=".v2-escada" /></h2>
      ...
      <div className="v2-escada-faixa" ref={faixaRef}>
        {patentes.map((r) => {
          ...
          const ehAtual = atual ? r.name === atual : false;
          return (
            <div key={r.name} className={`v2-escada-item ${conquistada ? "ok" : ""} ${ehAtual ? "atual" : ""}`} title={`${r.name} · ${r.min.toLocaleString("pt-BR")} pts`}>
              {r.icon ? <img src={r.icon} alt="" loading="lazy" className={r.brilha ? "brilha" : ""} onError={(e) => { e.currentTarget.style.visibility = "hidden"; }} /> : <span className="v2-escada-sem" />}
              <b>{r.name}</b>
              <small>{ehAtual ? "você está aqui" : conquistada ? `${r.min.toLocaleString("pt-BR")} pts` : `faltam ${(r.min - pontos).toLocaleString("pt-BR")}`}</small>
            </div>
          );
        })}
      </div>
