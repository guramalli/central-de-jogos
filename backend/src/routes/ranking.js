...
router.get("/monthly/:gameKey", requireAuth, async (req, res) => {
  ...
  const resposta = await cacheOuBuscar(cacheKey, 20, async () => {
    ...
    return scores.map((s, idx) => ({
      ...
      //
      // O `userId` é omitido de propósito: passá-lo dispara a checagem de
      // patente exclusiva, que pergunta "essa pessoa é a dona do topo AGORA?"
      // — e "agora" é sempre o mês corrente. Resultado: quem foi campeão em
      // agosto perdia a Coroa Imperial de Ouro no histórico assim que
      // setembro começava, porque já não era o detentor vigente.
      //
      // Aqui a exclusividade é decidida pela POSIÇÃO naquele mês: o primeiro
      // colocado é, por definição, quem tinha o topo. Os demais recebem a
      // patente logo abaixo, que é exatamente a regra do jogo.
